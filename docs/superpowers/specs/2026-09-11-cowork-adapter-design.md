...
| Store | `<Claude app data>/local-agent-mode-sessions/<account>/<org>/` — app data is `~/Library/Application Support/Claude` (macOS), `$XDG_CONFIG_HOME/Claude` (Linux). On Windows, `%APPDATA%\Claude`, or — for a Microsoft Store (MSIX) install, which redirects `%APPDATA%` into `…\Packages\<family>\LocalCache\Roaming` — whichever `%LOCALAPPDATA%\Packages\Claude_*\LocalCache\Roaming\Claude` candidate actually holds `local-agent-mode-sessions`, falling back to the classic path; mirrors `windowsDataDir()` in `claude-code.mjs`. Only account/org folders whose names are UUIDs are entered. `BOT_CROSSING_COWORK_SESSIONS` overrides the store root (tests, relocated installs). |
| What appears | **Interactive** sessions (no `scheduledTaskId`, `sessionType !== 'scheduled'`) whose `lastActivityAt` is within the last **30 days**: one thread each. **Enabled routines** (`enabled: true` in `scheduled-tasks.json`): one thread per routine, id `claude-cowork:task:<orgId>:<taskId>`, built from its runs inside the same 30-day window — title, error, size, model and liveness from the latest run, `createdAt` from the earliest. Runs of disabled routines, and routines with no run in the window, do not appear. A routine stays on the map even when its latest run is individually archived — only turning the routine off removes it (see Thread fields). |
| Placement | The first entry of `userSelectedFolders` (the run's, else the routine's in `scheduled-tasks.json`) that is an existing directory, resolved with `path.resolve` so a trailing separator or a `.`/`..` segment can't split one real folder into two zone names: `project` = its basename, `projectPath` = `cwd` = that resolved folder — so a Cowork astronaut shares the zone of any Claude Code thread in the same folder. None → `project: 'Cowork'`, `projectPath: ''`, `cwd: ''` (one shared zone; the HUD greys its folder buttons). |
...
| Thread fields | `lastActivityAt` = the record's (for a routine, its latest run's); `sizeBytes` = transcript bytes (0 if none); `hasError`, `starred`, `model` from the record; `routine` = task id or `''`; `unread: false` and `lastFocusedAt: 0` (Cowork keeps no focus history — unknowable, not true); `source: 'cowork'`; `worktree`, `gitBranch`, `effort`, `prState`, `preview` empty. `archived`: for an interactive session, `isArchived === true` from its own record; for a routine, always `false` — a routine is retired by disabling it (dropped under "What appears"), not by whichever run happens to be latest having been archived. |
| Running | Live only if the session's registry names a pid on this machine (`pidDomain` equal to `process.platform`, record `hostLoopMode === true`) that `kill(pid, 0)` accepts or answers EPERM, **and** the transcript moved in the last 30 minutes; the registry is only read once that freshness check on the transcript passes, so a long-cold thread is never probed. A registry without `pidDomain` is not probed. |
