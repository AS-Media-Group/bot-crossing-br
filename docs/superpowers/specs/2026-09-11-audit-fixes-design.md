...
| B3 | Vite's `/__open-in-editor` answers a cross-site GET. Vite serves repo files outside `/api` (`data/colony.json`, `.claude/*`). | Plugin middleware 404s the editor route. `server.fs = { strict, allow: [repo], deny: [defaults…, '**/data/colony.json*', '**/.claude/**'] }` — patterns containing `/` must be globstar-anchored or Vite ignores them. |
